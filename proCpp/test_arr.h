...
private:
    vector<int> preSum;
public:
    explicit NumArray303(vector<int> &nums) {
        // preSum[0] = 0, 便于计算包括索引为0的元素的前缀和
        preSum = vector<int>(nums.size() + 1, 0);
        // 计算并记录前缀和
        for (int i = 1; i < preSum.size(); i++) {
            preSum[i] = nums[i - 1] + preSum[i - 1];    // 代表区间[0, i-1]的前缀和
        }
    }
...
public:
    int subarraySum(vector<int> &nums, int k) {
        ...
        int res = 0;
        unordered_map<int, int> m;  // 记录每个前缀和的个数
        m[0] = 1;  // preSum[0]=0, 前缀和为0的个数初始化为1, 每个包含首元素的子数组需要使用preSum[0]计算前缀和
        for (int i = 1; i < preSum.size(); i++) {
            int need = preSum[i] - k;   // 找到是否存在闭区间前缀和为k-当前前缀和
            if (m.count(need) != 0) {
                res += m[need];
            }
            m[preSum[i]]++;     // 记录闭区间[0,i]的累加和
        }
        return res;
    }
};
